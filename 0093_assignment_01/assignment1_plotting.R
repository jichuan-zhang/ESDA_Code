...
income =
  tm_shape(lsoa_all, bbox = bbox_aux)+
...
# Second Map - Number of EV Charger
map2 =
  charging_ports_count+
  legends2+
  tm_layout(legend.show = TRUE,
...
map3 =
  ...
  tm_layout(legend.show = TRUE,
            legend.position = c("left",0.58),
            legend.title.fontface = "bold",
            title = "Income per EV Charger",
            title.position = c("center", "top"),
            title.size = 1.6)+
  tm_logo("Energy Institute Logo.png", height = 4, position = c("left", "top"))+
  other_stuff

# Fourth Map - Area Density
map4 =
  area_per_charging_port+
  legends4+
  tm_layout(legend.show = TRUE,
...
tmap_save(map3, filename = "Income per EV Charging Port 14.png", width = 1920, height = 1920)
